...
switch state
{
	...
	case states.slide:
		hsp = xscale * movespeed;
		if (place_meeting(x + sign(hsp), y, obj_solid) or place_meeting(x + sign(hsp), y, obj_noclimb) or scr_solid_slope(x + sign(hsp), y))
	...
	case states.slamground:
		...
		if (grounded and !place_meeting(x, y + vsp + 1, obj_destroyable) and !place_meeting(x, y + vsp + 1, obj_harddestroyable))
		{
			...
			if (scr_solid_slope(x, y - vsp - 1) && move == xscale)
			{
				sound_play_3d(sfx_land, x, y);
				sound_play_3d(sfx_slide, x, y);
				state = states.bounce;
		...
		case states.door:
			...
			slamspd = 0;
			movespeed = 0;
			if (floor(image_index) == (image_number - 1) && state != states.comingoutdoor)
			{
				image_speed = 0;
		...
		case states.comingoutdoor:
			...
			movespeed = 0;
			image_speed = 0.35
			if (floor(image_index) == (image_number - 1) && state != states.actor && state != states.door)
			{
				state = states.normal;
				sprite_index = spr_player_idle;
				image_index = 0;
		...
		case states.cock:
			...
			if (key_slap2 && global.cock)
			{
				with (instance_create(x, y, obj_cockthrown))
				{
					movespeed = 16;
...
if (global.key)
{
	keyX = x - 16 + lengthdir_x(75, keyBuffer)
	keyY = y + lengthdir_y(90, keyBuffer)
